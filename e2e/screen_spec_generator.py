...
SCREENS = [
    ...
    },
    {
        ...
        "features": [
            ...
            "タイトル変更（✏️ボタン→インライン編集）",
            "削除ボタン（確認ダイアログ付き）",
            "各ステップへのリンク（家族構成/財産/計算結果/遺言書シミュレーター）",
        ],
    ...
    },
    {
        ...
        "api": ["GET /estate-plans/{id}/calculate"],
    },
    {
        "id": "estate_will",
        "name": "遺言書シミュレーター",
    ...
    },
    {
        ...
        "features": [
            "タブ: 医療・介護 / 葬儀 / 形見分け / デジタル資産 / 緊急連絡先 / ペット / お気に入り / 家族へのメッセージ / 追悼メッセージ / ビデオメッセージ（入力済みタブは●ドットインジケーター表示）",
            ...
            "追悼メッセージタブ: 予約送信メッセージの作成・管理",
            "ビデオメッセージタブ: 大切な人へのビデオ録画アップロード・管理（デジタル遺品鍵開錠後に公開）",
        ],
        "api": ["GET /ending-note", "PUT /ending-note", "POST/DELETE /bequest-items /digital-assets /subscriptions /pets /emergency-contacts", "GET/POST/PUT/DELETE /scheduled-messages", "GET/POST/DELETE /video-messages"],
    },
    {
        "id": "digital_key",
        "name": "デジタルカギ設定",
        ...
        "features": [
            "信頼できる人の登録（最大3名・名前・メール）",
            "解除条件選択（1名申請で開錠 / 2名申請で開錠）",
            "解除キーURLのコピー（信頼者へ共有するワンタイムURL）",
            "信頼者向け解除申請ページ（/unlock/:id?token=... — ログイン不要）",
            "デッドマンスイッチ: 有効/無効切替・チェックイン間隔（30/60/90/180/365日）",
            "チェックインボタン（最終確認日時の更新）",
            "メモ欄（自由記入）",
            "保存ボタン",
        ],
        "api": ["GET /digital-key", "PATCH /digital-key", "POST /digital-key/checkin", "POST/DELETE /digital-key/trusted-persons", "POST /digital-key/trusted-persons/{id}/request"],
    },
    {
        "id": "account",
        "name": "アカウント設定",
    ...
    },
    {
        ...
        "features": [
            ...
            "写真の公開/非公開切替（🌐公開中/🔒非公開トグルボタン・詳細展開時に表示）",
            "公開URLコピーボタン（slug使用・正しいQR対応URL）",
            "保存ボタン",
        ],
    ...
    },
    {
        ...
        "features": [
            "故人名・生没年表示",
            "略歴・メッセージ",
            "写真ギャラリー（media_is_public=Trueの写真のみ表示）",
            "パスワード保護ページ（非公開設定時）",
        ],
        "api": ["GET /m/{slug}"],
    },
    {
        ...
        "features": [
            "メール通知有効/無効トグル（全通知の一括ON/OFF）",
            "通知先メールアドレス入力（未設定時はアカウントのメールに送信）",
            "年次見直しリマインダー月選択（1月〜12月から選択）",
            "未完了タスクリマインダートグル",
            "信頼者確認メールトグル（年1回、登録信頼者の有効性確認）",
        ],
        "api": ["GET /reminder-settings", "PUT /reminder-settings"],
    },
    {
        "id": "unlock",
        "name": "デジタル遺品鍵 解除申請ページ",
        "path": "/unlock/:personId",
        "desc": "信頼者が解除キーURLを使って開錠申請を送るページ（ログイン不要・公開）。",
        "features": [
            "URLパラメーターからpersonIdとtokenを取得",
            "申請の確認説明と注意書き表示",
            "「解除申請を送る」ボタン",
            "申請成功時: 開錠状況メッセージ表示",
            "エラー時: 無効URL・申請失敗メッセージ表示",
        ],
        "api": ["POST /digital-key/trusted-persons/{id}/request?token=..."],
    },
]
